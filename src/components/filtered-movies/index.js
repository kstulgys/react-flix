...
function MovieInfo({ movieId }) {
  ...
  return (
    <GridWrapper>
      <div className='w-100'>
        {!movieInfo ? <div> <h1>Loading...</h1> </div>
          :
          <div className='d-flex'>
            <div className='col-6 p-0'>
              <img src={movieInfo.poster_path} className='rounded' alt='poster' style={{ width: '100%' }} />
            </div>
    ...
    <GridWrapper>
      {filteredMovies.map(({ id, title, poster_path }) => {
        return (
          <div
            key={id}
            className="rounded col-12 px-0 mb-4 col-md-6 pr-md-4 col-lg-4"
          // style={{ width: "400px" }}
          >
            <Link to={`/${id}`}>
              <img
...
function GridWrapper({ children }) {
  return (
    <div className="d-flex flex-wrap px-0 mt-4 w-100">
      {children}
    </div>
  )
}
