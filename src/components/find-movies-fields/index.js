...
export default function FindMoviesFields() {
  ...
  return (
    <div className='bg-dark px-4 py-5  rounded'>
      ...
      <br />
      <Slider title='Year' min="1990" max="2019" currentValue={state.primary_release_year} onChange={handleReleaseYearChange} />
      <br />
      <Slider title='Rating' min="5" max="10" defaultVal="8" currentValue={state.vote_average} onChange={handleVoteChange} />
      <br />
      <br />
      <Button disabled={state.genreId === '10101'} onClick={handleSubmit} size='lg' className='w-100' theme="danger"><h5 className='m-0 text-light font-weight-bold'>Search</h5></Button>
    </div>
  )
}
